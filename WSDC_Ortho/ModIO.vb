...
Module ModIO
    ...
    Public Function g_IO_Execute_SQL(ByVal strSQL As String, ByRef blnReturnErrorCode As Boolean, ByVal strDatabaseType As String) As DataTable
        ...
        Catch ex As Exception
            ...
            'System.Web.HttpContext.Current.Response.Write("*SQL_ERROR*" & strSQL & "*SQL_ERROR*")
    ...
    Private Function IO_Execute_MSSQL(ByVal strSQL As String, ByRef ReturnCode As Boolean) As DataTable
        ...
        Try
            ...
            'If strSQL.Contains("sysdatabases") Then
            '    strConnectionString.Replace(g_ConnectionSchema, "master")
            ...
            If IsNothing(HttpContext.Current.Session("ERRORLOG")) Then
                HttpContext.Current.Session.Remove("SQLERROR")
            End If
            ...
            For i = 1 To g_intNumberOfRetriesToAccessDatabase
                ...
                If IsNothing(HttpContext.Current.Session("EMAILLOGSELECT")) Then
                    ' send sql email if we are not already in g_sendEmail logging function
                    Try
                        If Trim(g_EmailIOAddress) = "" Then
                        Else
                            Select Case True
                                Case g_EmailInserts And UCase(Trim(Left(strSQL, 6))).IndexOf("INSERT") > -1
                                    g_sendEmail(g_EmailIOAddress, g_SiteDisplayName & " - IO Insert: ", strSQL)
                                Case g_EmailUpdates And UCase(Trim(Left(strSQL, 6))).IndexOf("UPDATE") > -1
                                    g_sendEmail(g_EmailIOAddress, g_SiteDisplayName & " - IO Update: ", strSQL)
                                Case g_EmailDeletes And UCase(Trim(Left(strSQL, 6))).IndexOf("DELETE") > -1
                                    g_sendEmail(g_EmailIOAddress, g_SiteDisplayName & " - IO Delete: ", strSQL)
                            End Select
        ...
        Catch ex As Exception
            ...
            ReturnCode = False
            ' set session variable with error message if we are not processing error logging
            If IsNothing(HttpContext.Current.Session("ERRORLOG")) Then
                HttpContext.Current.Session("SQLERROR") = ex.Message
            End If
    ...
    Public Sub g_GetColumns(ByVal tableName As String, ByRef nvcColumnLength As NameValueCollection,
                            ByRef nvcColumnType As NameValueCollection,
                            ByRef nvcColumnDescription As NameValueCollection,
                            ByRef nvcColumnDDLTableName As NameValueCollection,
                            ByRef nvcColumnDDLValue As NameValueCollection,
                            ByRef nvcColumnDDLText As NameValueCollection,
                            ByRef nvcColumnPwd As NameValueCollection,
                            ByRef nvcShowColumnInGrid As NameValueCollection,
                            ByRef nvcColumnShowInPopup As NameValueCollection,
                            ByRef nvcHidden As NameValueCollection,
                            ByRef nvcDisabled As NameValueCollection,
                            ByRef nvcColumnIndex As NameValueCollection,
                            ByRef nvcColumnRequired As NameValueCollection,
                            ByRef nvcColumnEmail As NameValueCollection,
                            ByRef nvcColumnPhone As NameValueCollection,
                            ByRef nvcColumnTotal As NameValueCollection,
                            ByRef nvcColumnDisplayLocn As NameValueCollection,
                            ByRef nvcColumnUnique As NameValueCollection,
                            ByRef nvcColumnNames As NameValueCollection,
                            ByRef nvcRegExpPattern As NameValueCollection,
                            ByRef nvcRegExpMessage As NameValueCollection,
                            ByRef nvcMinValue As NameValueCollection,
                            ByRef nvcMaxValue As NameValueCollection,
                            ByRef nvcColumnDefaultValue As NameValueCollection,
                            ByRef m_nvcaSign As NameValueCollection,
                            ByRef m_nvcpSign As NameValueCollection,
                            ByRef m_nvcPercentage As NameValueCollection,
                            ByRef m_nvcShowSeconds As NameValueCollection,
                            ByRef m_nvcImageMinMax As NameValueCollection,
                            ByRef m_nvcImageHover As NameValueCollection,
                            ByRef m_nvcColumnDDLFilter As NameValueCollection)
        ...
        ' 01/06/15 T3 moved check for table/view to common function, used in multiple locations
        Dim strJoinTo As String = determineViewOrTable(tableName, False)
        Dim strSQL As String = "SELECT isnull(sep.name, '') [extName], isnull(sep.value, '') [extValue], sc.name [Column], sc.max_length [MaxLength], ic.DATA_TYPE, " &
                               "CAST(ISNULL(ic.numeric_precision, '') AS char(5)) as numeric_precision, CAST(ISNULL(ic.numeric_scale, '') AS char(5)) as numeric_scale, column_default " &
                               "FROM " & strJoinTo & " st " &
        ...
        Dim strImageFieldsDelim As String = ""
        For Each rowColumns As DataRow In tblColumns.Rows
            ...
            Else
                ...
                Select Case UCase(rowColumns("extName"))
                    ...
                    Case "DDLFILTER"
                        m_nvcColumnDDLFilter(rowColumns("Column")) = rowColumns("extValue")
                    Case "PWD"
                    ...
                    Case "SHOWINPOPUP"
                        nvcColumnShowInPopup(rowColumns("Column")) = rowColumns("extValue")
                    Case "HIDDEN"
                    ...
                    Case "IMAGE"
                        strImageFieldsList &= strImageFieldsDelim & rowColumns("Column")
                        strImageFieldsDelim = "~~"
                       ' nvcColumnType(rowColumns("Column")) = "image"
                    Case "IMAGESIZE"
                        m_nvcImageMinMax(rowColumns("Column")) = rowColumns("extValue")
                    Case "IMAGEHOVER"
                        m_nvcImageHover(rowColumns("Column")) = rowColumns("extValue")
                End Select
                ...
                End If
                If IsNothing(nvcColumnShowInPopup(rowColumns("Column"))) Then
                    nvcColumnShowInPopup(rowColumns("Column")) = "TRUE"
                End If
                If UCase(nvcColumnShowInPopup(rowColumns("Column"))) = "TRUE" Then
                    '04/30/15 Add column to nvcColumnName
                    nvcColumnNames(rowColumns("Column")) = rowColumns("Column")
                End If
        ...
        If strImageFieldsList = "" Then
        Else
            Dim arrImageFieldList() As String = Split(strImageFieldsList, "~~")
            For Each column In arrImageFieldList
                nvcColumnType(column) = "image"
            Next
        End If

        intIndex = 0
        For Each columnId In nvcShowColumnInGrid.AllKeys
            If nvcShowColumnInGrid(columnId) = True Then
                nvcColumnIndex(columnId) = intIndex
                intIndex += 1
            End If
        Next
    ...
    Public Sub g_logError(ByVal strSQL As String, ByVal strErrorMessage As String)
        If g_errorLog Then
            HttpContext.Current.Session("ERRORLOG") = "1"
            Dim userRecid As Integer = -1
            If IsNothing(System.Web.HttpContext.Current.Session("user_link_id")) Then
            Else
                userRecid = System.Web.HttpContext.Current.Session("user_link_id")
            ...
            Else
                ...
                If tblTableCheck.Rows(0)("tblCount") = 0 Then
                    'we need to create autUdates_log table
                    Dim strCreateTbl As String = "CREATE TABLE [dbo].[error_log](" &
                  "      [recid] [int] IDENTITY(1,1) NOT NULL," &
                  "      [sqlStatement] [varchar](8000) NULL," &
                  "      [errorMessage] [varchar](8000) NULL," &
                  "      [timestamp] [datetime] NULL," &
                  "      [user_recid] [int] NULL," &
                  "      [frmUrl] [varchar](8000) NULL," &
            ...
            If IsNothing(HttpContext.Current.Session("frmUrl")) Then
                strUrl = HttpContext.Current.Request.Url.AbsoluteUri
            Else

                strUrl = HttpContext.Current.Session("frmUrl")
            End If
            Dim arrUrl = Split(strUrl, "?E=")
            If arrUrl.Count > 1 Then
                strUrl = arrUrl(0) & "?E=" & g_Decrypt(arrUrl(1))
            End If

            IO_Execute_MSSQL("Insert into error_log (sqlStatement, errorMessage, timestamp, user_recid, frmUrl) " &
                             " VALUES ('" & Left(strSQL.Replace("'", "''"), 8000) & "', '" & Left(strErrorMessage.Replace("'", "''"), 8000) & "', getdate() ," & userRecid & ", '" & strUrl & "')", False)


            HttpContext.Current.Session.Remove("ERRORLOG")
        End If

        If g_EmailErrors Then
            g_sendEmail(g_EmailIOAddress, g_SiteDisplayName & " - IO Error: ", strSQL & vbCrLf & vbCrLf & strErrorMessage)
        End If
    End Sub
    '**********************************SQL Insert**********************************
    ...
    Private Function IO_Execute_SQLInsert(ByRef TableName As String, ByRef nvcFieldValues As NameValueCollection, _
        ...
        Dim blnFieldsOk As Boolean = True
        ...
        Do Until Not blnRetry
            ...
            For Each strColumnName As String In nvcFields.Keys
                ...
                Else
                    ...
                    Catch ex As Exception
                        ...
                        g_logError(strInsertSQL, "Dear Programmer -- You have data problems -- Invalid Data Sent")
                        blnFieldsOk = False
                    End Try
            ...
            If strValues = " VALUES (" AndAlso strFields = " (" Then
                blnFieldsOk = False
            Else
            ...
            Else
                ...
                For Each strcolumn As String In m_nvcConvertedInserts.AllKeys
                    ...
                    strDelimiter = ", "
                Next
                g_logError(strInsertSQL, "The following fields were provided but don't seem to exist in " & TableName & " (Could be database communication error): " & strUnusedColumnsProvided)
                blnFieldsOk = False
        ...
        If blnFieldsOk Then
            Try
                ...
                Else
                    ...
                    System.Web.HttpContext.Current.Session("NewMSSQLRECID") = LastRecId
                End If
            ...
            Catch ex As Exception
                g_logError(strInsertSQL, ex.Message)
            ...
            'End If
        Else
            ' 3/10/16 cpb if there is an issue with fields this never got turned on.
            ReturnACompletionCode = False
        End If
    ...
    Private Function IO_Execute_SQLDelete(ByRef TableName As String, ByVal WherePhrase As String, _
        ...
        ' perform delete
        Dim strSQLToDeleteRecords As String = "Delete FROM " & TableName &
                        IIf(Trim(WherePhrase) = "", "", " WHERE " & WherePhrase)
        Try
            g_IO_Execute_SQL(strSQLToDeleteRecords, ReturnACompletionCode)
        Catch ex As Exception
            g_logError(strSQLToDeleteRecords, ex.Message)
        End Try
    ...
    Private Sub IO_Execute_SQLUpdate(ByRef TableName As String, ByRef nvcFieldValues As NameValueCollection, _
        ...
        Else
            ...
            For Each rowLiveDatabaseRecordBeforeUpdate As DataRow In tblLiveDatabaseRecordBeforeUpdate.Rows
                ...
                ' 02/24/16 get column types so we can test for 'time'
                Dim nvcColumnLength As New NameValueCollection
                Dim nvcColumnType As New NameValueCollection
                Dim nvcColumnDescription As New NameValueCollection
                Dim nvcColumnDDLTableName As New NameValueCollection
                Dim nvcColumnDDLValue As New NameValueCollection
                Dim nvcColumnDDLText As New NameValueCollection
                Dim nvcColumnPwd As New NameValueCollection
                Dim nvcColumnShowInGrid As New NameValueCollection
                Dim nvcColumnShowInPopup As New NameValueCollection
                Dim nvcHidden As New NameValueCollection
                Dim nvcDisabled As New NameValueCollection
                Dim nvcColumnIndex As New NameValueCollection
                Dim nvcColumnRequired As New NameValueCollection
                Dim nvcColumnEmail As New NameValueCollection
                Dim nvcColumnPhone As New NameValueCollection
                Dim nvcColumnTotal As New NameValueCollection
                Dim nvcColumnDisplayLocn As New NameValueCollection
                Dim nvcColumnUnique As New NameValueCollection
                Dim nvcColumnNames As New NameValueCollection
                Dim nvcColumnRegExpPattern As New NameValueCollection
                Dim nvcColumnRegExpMessage As New NameValueCollection
                Dim nvcMinValue As New NameValueCollection
                Dim nvcMaxValue As New NameValueCollection
                Dim nvcColumnDefaultValue As New NameValueCollection
                Dim nvcaSign As New NameValueCollection
                Dim nvcpSign As New NameValueCollection
                Dim nvcPercentage As New NameValueCollection
                Dim nvcShowSeconds As New NameValueCollection
                Dim nvcColumnDDLFilter As New NameValueCollection
                Dim nvcImageMinMax As New NameValueCollection
                Dim nvcImageHover As New NameValueCollection

                g_GetColumns(TableName, nvcColumnLength, nvcColumnType, nvcColumnDescription, nvcColumnDDLTableName,
                     nvcColumnDDLValue, nvcColumnDDLText, nvcColumnPwd, nvcColumnShowInGrid, nvcColumnShowInPopup, nvcHidden,
                     nvcDisabled, nvcColumnIndex, nvcColumnRequired, nvcColumnEmail, nvcColumnPhone,
                     nvcColumnTotal, nvcColumnDisplayLocn, nvcColumnUnique, nvcColumnNames,
                     nvcColumnRegExpPattern, nvcColumnRegExpMessage, nvcMinValue, nvcMaxValue, nvcColumnDefaultValue,
                     nvcaSign, nvcpSign, nvcPercentage, nvcShowSeconds, nvcColumnDDLFilter, nvcImageMinMax, nvcImageHover)
                ...
                For Each colColumnName As DataColumn In tblLiveDatabaseRecordBeforeUpdate.Columns
                    ...
                    Dim blnSkipField As Boolean = False
                    'If (Not IsDBNull(rowLiveDatabaseRecordBeforeUpdate(strColumnName)) AndAlso
                    '    Trim(rowLiveDatabaseRecordBeforeUpdate(strColumnName)) = Trim(nvcFieldValues(strColumnName))) Or
                    '    nvcFieldValues(strColumnName) Is Nothing Then
                    'Else

                    If nvcColumnType(strColumnName).Contains("time") AndAlso LCase((nvcColumnType(strColumnName))) <> "datetime" Then
                        Dim strTimeFormat As String = "hh:mm tt"
                        If IsNothing(nvcShowSeconds(strColumnName)) Then
                        Else
                            If nvcShowSeconds(strColumnName) = True Then
                                strTimeFormat = "hh:mm:ss tt"

                            End If
                        End If
                        'Format(CDate("01/01/01 " & tblOpenOrder.Rows(0)("time").ToString()), strTimeFormat)
                        If (Not IsDBNull(rowLiveDatabaseRecordBeforeUpdate(strColumnName)) AndAlso
                            Trim(Format(CDate("01/01/01 " & rowLiveDatabaseRecordBeforeUpdate(strColumnName).ToString()), strTimeFormat)) = Trim(nvcFieldValues(strColumnName))) Or
                            nvcFieldValues(strColumnName) Is Nothing Then
                            blnSkipField = True
                        End If
                    Else
                    ...
                    Else
                        ...
                        If blnBuildUpdateSQL Then
                            Select Case strColumnType
                                ...
                                Case "BOO"
                                    Try
                                        If Trim(nvcFieldValues(strColumnName)) = "True" AndAlso Not IsDBNull(rowLiveDatabaseRecordBeforeUpdate(strColumnName)) AndAlso Trim(rowLiveDatabaseRecordBeforeUpdate(strColumnName)) = 1 Or
                                        ...
                                        End If
                                    Catch ex As Exception
                                        If Trim(nvcFieldValues(strColumnName)) = "True" AndAlso Not IsDBNull(rowLiveDatabaseRecordBeforeUpdate(strColumnName)) AndAlso Trim(rowLiveDatabaseRecordBeforeUpdate(strColumnName)) = True Or
            ...
            Else
                ...
                'Edited 09/15/15 T3 - check if last_modified_date or last_modified_recid exist

                If IsNothing(tblLiveDatabaseRecordBeforeUpdate.Columns("last_modified_date")) Then
                Else
                    strUpdateSQL &= strDelimiter & "last_modified_date ='" & Date.Now & "'"
                    strDelimiter = ","
                End If

                If IsNothing(tblLiveDatabaseRecordBeforeUpdate.Columns("last_modified_recid")) Then
                Else
                    strUpdateSQL &= strDelimiter & "last_modified_recid = " & g_UserRecId
                    strDelimiter = ","
                End If
                ...
                Try
                    Call g_IO_Execute_SQL(strUpdateSQL, ReturnACompletionCode)
                Catch ex As Exception
                    g_logError(strUpdateSQL, ex.Message)
                End Try
